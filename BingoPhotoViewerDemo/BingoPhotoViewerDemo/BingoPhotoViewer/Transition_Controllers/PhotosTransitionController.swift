...
@objc(PhotosTransitionControllerMode)
enum PhotosTransitionControllerMode : Int {
    case presenting, dismissing
}

@objc(PhotosTransitionControllerDelegate)
protocol PhotosTransitionControllerDelegate {
    @objc(transitionController:didFinishAnimatingWithView:transitionControllerMode:)
    func transitionController(_ transitionController: PhotosTransitionController,
                              didFinishAnimatingWith view: UIImageView,
                              transitionControllerMode: PhotosTransitionControllerMode) -> Void
}


@objc(PhotosTransitionController)
class PhotosTransitionController: NSObject{
...
extension PhotosTransitionController{
...
extension PhotosTransitionController : UIViewControllerAnimatedTransitioning{
    ...
    fileprivate func animatePresentation(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        UIView.animate(
            cornerRadius: transitionDuration(using: transitionContext) * FadeInOutTransitionRatio,
...
extension PhotosTransitionController : UIViewControllerInteractiveTransitioning{
    func startInteractiveTransition(_ transitionContext: UIViewControllerContextTransitioning) {
        dismissalTransitionContext = transitionContext
        ...
        var photosViewController: PhotosViewController
        if
            let from = from as? PhotosViewController {
            photosViewController = from
        } else {
            guard
                let childViewController = from.childViewControllers.filter({ $0 is PhotosViewController }).first as? PhotosViewController
                else {
                    assertionFailure("Could not find AXPhotosViewController in container's children.")
                    return
            }
        ...
        guard
            let imageView = photosViewController.currentPhotoViewController?.zoomingImageView.imageView as UIImageView?
            else {
                assertionFailure("No. ಠ_ಠ")
                return
        }
        ...
        if forceImmediateInteractiveDismissal {
        ...
        imageViewOriginalSuperview = imageView.superview
        ...
        let overlayView = photosViewController.overlayView
        overlayViewOriginalSuperview = overlayView.superview
        ...
        topStackContainerInitialOriginY = overlayView.topStackContainer.frame.origin.y
        bottomStackContainerInitialOriginY = overlayView.bottomStackContainer.frame.origin.y
        
        from.view.alpha = 0
        
        if canPerformContextualDismissal() {
            guard let referenceView = transitionInfo.endingView else {
                assertionFailure("No. ಠ_ಠ")
                return
            }
...
extension PhotosTransitionController : UIGestureRecognizerDelegate{
    ...
    func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        guard !(zoomingImageView?.isScrollEnabled ?? true) &&
            (pagingConfig.orientation == .horizontal ||
                (pagingConfig.orientation == .vertical &&
